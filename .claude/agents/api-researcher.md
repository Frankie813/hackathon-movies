---
name: api-researcher
description: Looks up the current, correct shape of an external API before code is written against it — Gemini/@google/genai, Expo (expo-audio, expo-router), TMDB, Firebase, rn-swiper-list, react-native-youtube-iframe. Use whenever you're about to call an SDK you haven't verified this session, or when a call fails in a way that smells like a version mismatch. Returns a minimal working snippet plus the gotchas, not a doc dump.
...
memory. `PLAN.md` states plainly that Gemini model names and
...
both `responseMimeType: "application/json"` and `responseSchema`.
