...
**MVP (P0):** swipe deck of trailer cards · like/dislike updates taste vector and re-ranks · join group by code → mutual match · one Gemini feature ("why") · **(v2)** `.tech` domain registered, prize submission checklist. Watch-later: explicit saves and group matches persist and show in a Saved tab (#41, #87).
...
41. **[P1] Saved tab (watch later)** — List of saved movies and group matches from Firestore (`users/{uid}/watchlist`, `users/{uid}/matches`): poster, title, where-to-watch badge. Long-press to remove. A like is *not* a save (revised by #87): a Watch Later button beside the deck's Like/Pass makes the save explicit, and saves and likes in one tap. *AC:* A save appears in the tab within a second. A group match appears for every member after the reveal. *Est:* 1.5h. *Dep:* #18, #17. *Owner:* A. *Phase:* 3.
